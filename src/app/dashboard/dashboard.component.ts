...
export class DashboardComponent implements OnInit {
    ...
    this.defaultService.getAllDepots().subscribe((data) => {
      ...
      $("#treegrid").fancytree({
        ...
        table: {
          ...
          nodeColumnIdx: 0 // render the checkboxes into the 1st column
        },
        source: [
  {
  "title": "Books",
  ...
  "lazy": true
        ...
        renderColumns: function(event, data) {
          ...
          $tdList.eq(1).text(node.data.qty);
          $tdList.eq(2).text(node.data.type);
          $tdList.eq(3).text(node.data.year);
          $tdList.eq(4).text(node.data.qty);
          $tdList.eq(5).text(node.data.type);
          $tdList.eq(6).text(node.data.qty);
          $tdList.eq(7).text(node.data.year);
          $tdList.eq(8).text(node.data.type);
          $tdList.eq(9).text(node.data.year);
          $tdList.eq(10).text(node.data.qty);
          $tdList.eq(11).text(node.data.year);
          $tdList.eq(12).text(node.data.type);
